...
#histogram of men's heights and 
```{r}

hist(mdims$hgt, xlab = "Height of Men in cm", main = "Histogram For Men's Height")
```
    
# histogram of women's heights    
```{r}
ggplot(fdims, aes(hgt, color='red') ) + geom_histogram(binwidth = 5 , fill='white' )+xlab("Height of Women in cm")

```    


### Answers: Both the men’s and women's height look to be normalzied. The latter seems to have a larger concentration within one std deviation than the former.    
...
## Answer: Yes, it does appear that the data follows a nearly normal distribution.
...
## The plots produced above seem to have more elements on the line than the original plot. But similar to original plot they seem to have similar outliers. So overall, the female plot is nearly normal.
...
## Using the above observations we see that there are a lot of outliers and deviations from the line, hence it seems the female weight is not nearly normal.
...
##  heights probability less then 150

```{r}
Exp.height <- pnorm(q = 150, mean = fhgtmean, sd = fhgtsd)
print(paste0("Expected prob from Normal distribution of Female Height: ", Exp.height))
```
 
```{r}
Actual.height <- sum(fdims$hgt < 150) / length(fdims$hgt)
print(paste0("Actual value from dataset for Female Height:  ", Actual.height))
```
```{r}
Exp.height-Actual.height
```
 
 
 
##  Weight probability higher then 120 

```{r}
Exp.weight <- pnorm(q = 120, mean = fhgtmean, sd = fhgtsd)
print(paste0("Expected prob from Normal distribution of Female Height: ", Exp.weight))
```
 
```{r}
Actual.weight <- sum(fdims$wgt < 120) / length(fdims$wgt)
print(paste0("Actual value from dataset for Female Height:  ", Actual.weight))
```
```{r}
Exp.height-Actual.weight
```
## Based on above it seems the female height values are more in line with expectations from normal distribution than the weight’s.
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter ___B_.
    

    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter __C_.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
   ...
   # Because they are variable with discrete units not continuous. Thats what gives them the stepwise pattern.
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    your findings.
 #It is right skewed based on the figures below. 
